...
object PrepareTweets {
  ...
  def startTweetsStreaming(): Unit = {
    logger.info( s"""Starting Streaming at: ${LoadConf.sparkConf.getString("powertrack.twitterStreamingDataPath")}""") // scalastyle:ignore
  ...
  def createContext(): StreamingContext = {
    ...
    val ssc = new StreamingContext(ApplicationContext.sparkContext,
      Seconds(LoadConf.sparkConf.getInt("powertrack.streamingBatchTime")))

    // Filtering file's path in order to avoid _copying files
    val tweetsStreaming = ssc.fileStream[LongWritable, Text, TextInputFormat](LoadConf.sparkConf.getString("powertrack.twitterStreamingDataPath"), // scalastyle:ignore
      (p: Path) => {
        if (p.getName().toLowerCase().endsWith(LoadConf.sparkConf.getString("powertrack.fileExtension"))) true else false // scalastyle:ignore
    ...
    try {
      ...
      if (tweets.count() > 0) {
        ApplicationContext.sqlContext.read.json(tweets)
          ...
          .options(Map("pushdown" -> "true", "es.nodes" -> LoadConf.esConf.getString("bindIP"),
          "es.port" -> LoadConf.esConf.getString("bindPort")))
          .save( s"""${LoadConf.esConf.getString("powertrackIndexName")}/${LoadConf.esConf.getString("esType")}""") // scalastyle:ignore
      }
      else {
