...
sbt compile
sbt 'project redrock-decahose' compile package assembly

# Changes the location where spark is being launched so it creates its own metastore_db
cd $REDROCK_HOME/twitter-decahose

echo "============ Running Decahose Spark Streaming =============="
#run program on cluster
HOSTNAME="$(/bin/hostname -f)"
nohup $SPARK_HOME/bin/spark-submit --driver-memory 2g --master spark://$HOSTNAME:7077 --packages org.elasticsearch:elasticsearch-spark_2.10:2.1.1 --class com.decahose.Application $REDROCK_HOME/twitter-decahose/target/scala-2.10/redrock-twitter-decahose.jar > $REDROCK_HOME/twitter-decahose/nohup-decahose.out&
